          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  changepirce="0"
                  :class="{ active: spuSaleAttrValue.isChecked == 1 ? 1 : 0 }"
                  v-for="(
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addOrUpdateSkuNum">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 改变售卖属性中的高亮
    /* 
      实现思路：售卖属性中的高亮是取决于每一项的isChecked属性的，
                因此可以拿到整个列表，将所有的isChecked置为0，再将点击的那一项置为1(排他)
      需要拿到的数据： 需要拿到当前点击项的对象，以及整个列表
    */
    ...
    // 数量商品数量输入框
    changeSkuNum(event){
      // 处理输入框非法内容  -- 小于0或者有字母中文等
      // 将输入框的值作乘法，如果其中含有非Number的，则value的值就是NAN
    ...
    // 加入购物车
    async addOrUpdateSkuNum(){
      // 派发actions
      /* 
      派发后执行的getAddOrUpdateSkuNum是用async和await包裹的函数，这个函数会返回一个Promise，
      可以在这个函数内部判断请求成功的结果并返回对应的结果，然后在这里接收返回结果，就可以知道请求是否成功
      */

     try {
      await this.$store.dispatch('getAddOrUpdateSkuNum',{skuId:this.$route.params.skuId,skuNum: this.skuNum})
