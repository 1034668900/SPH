...
import { reqGetOrderInfo, searchResultPay } from "@/api";
export default {
  name: "Pay",
  data() {
    return {
  ...
  methods: {
    // 获取交易订单信息
    async getOrderInfo() {
      let result = await reqGetOrderInfo(this.$route.query.orderId);
    ...
    async open() {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          // 查询用户订单支付状态
          let result = await searchResultPay(this.orderInfo.orderId);
          // 判断支付状态
          if (result.data.code == 200) {
            // 支付成功-关闭延时器 -- 隐藏支付遮罩层 -- 保存
            clearInterval(this.timer);
            this.timer = null;
            // 保存支付成功的状态码 code
            this.payCode = result.data.code;
            // 关闭弹出框
            this.$msgbox.close();
            // 跳转到支付成功的路由
            this.$router.push("/paysuccess");
  ...
  computed: {
    // 需要支付的总价
    totalFee() {
      return this.orderInfo.totalFee;
    },
  },
};
</script>
